...
//     auth context can clear stale sessions and redirect to /signin
// ============================================================================

import appConfig from '../config/app.json'
import { ApiError, extractApiErrorMessage } from './api-error.js'

const TOKEN_STORAGE_KEY = import.meta.env.VITE_AUTH_TOKEN_KEY || 'chinasky_user_token'
const DEFAULT_TIMEOUT_MS = Number(import.meta.env.VITE_API_TIMEOUT_MS) || 30000

// Resolve the base URL. Priority:
//   1. Vite env var VITE_API_BASE_URL (set per environment)
//   2. apiBaseUrl in config/app.json (legacy)
//   3. "/api" relative path (works with the Vite dev proxy)
 ...
 * @property {unknown} [body]              JSON-serializable body. Use `rawBody` to bypass.
 * @property {BodyInit} [rawBody]          Pre-serialized body (FormData, Blob, etc.)
 * @property {Record<string,unknown>} [query]
 * @property {boolean} [auth]              Default true: attach Authorization header.
...
export async function request(path, options = {}) {
  ...
  // Wire up timeout via AbortController. If the caller passed their own
  // signal, link the two so either can cancel the request.
  ...
  if (!contentType.includes('application/json')) {
    // Some endpoints return plain text or empty body — be defensive.
